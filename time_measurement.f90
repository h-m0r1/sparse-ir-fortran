program main
    ...
    call time_fermion1(.false., .false., num)
    ...
    ! fermion
    subroutine time_fermion1(preset, positive_only, num)
        logical, intent(in) :: preset
        logical, intent(in) :: positive_only
        integer, intent(in) :: num
        type(IR) :: ir_obj
        ...
        complex(kind(0d0)),allocatable :: giv(:,:), gl_matsu(:, :), gl_tau(:, :), gtau(:, :), &
            gtau_reconst(:, :), giv_reconst(:, :), giv1(:), gl_matsu1(:) 
        double precision,allocatable :: gl_matsu_d(:, :), gl_tau_d(:, :), gtau_d(:, :), &
            gtau_reconst_d(:, :)
        integer n, t, l, i
        integer:: time_begin_c, time_end_c, CountPerSec, CountMax
        ...
        allocate(gl_tau_d(1, ir_obj%size))
        allocate(gtau_reconst_d(1, ir_obj%ntau))
        allocate(giv1(ir_obj%nfreq_f))
        allocate(gl_matsu1(ir_obj%size))
        ...
        call system_clock(time_end_c)
        write(*,*) real(time_end_c - time_begin_c)/CountPerSec," sec (fit_matsubara_f_zz_1)"
        call sleep(1)
