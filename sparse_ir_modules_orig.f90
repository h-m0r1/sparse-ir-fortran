...
subroutine fit_matsubara_f_zd(obj, arr, res)
    ...
    !ut(ns, m) * arr(l1, m) -> ut_arr(ns, l1)
    ut_arr(:, :) = czero
    call zgemm("n", "t", ns, l1, m, cone, obj%uhat_f%ut, ns, arr_tmp, l1, czero, ut_arr, ns)
    do j = 1, ns
        do i = 1, l1
            ut_arr(j, i) = ut_arr(j, i) * obj%uhat_f%inv_s(j)
        end do
...
subroutine fit_matsubara_b_zd(obj, arr, res)
    ...
    !ut(ns, m) * arr(l1, m) -> ut_arr(ns, l1)
    ut_arr(:, :) = czero
    call zgemm("n", "t", ns, l1, m, cone, obj%uhat_b%ut, ns, arr_tmp, l1, czero, ut_arr, ns)
    do j = 1, ns
        do i = 1, l1
            ut_arr(j, i) = ut_arr(j, i) * obj%uhat_b%inv_s(j)
        end do
